...
$location = new Ride();  
$locate = $location->completeRides($Connection->con);
...
<?php
    if(isset($_POST['filter'])){
        if(!empty($_POST['select'])){
            ...
            $filter = $location->filterComplete($selected, $Connection->con);
        }
        else{
        echo("not found");
        }
    ...
    if(isset($_POST['sort'])){
        if(!empty($_POST['select'])){
            ...
            // echo $sort;
            // echo "</br>";
            $sorted = $location->sortComplete($sort, $Connection->con);
            $fare = $location->fareComplete($sort, $Connection->con);
            $cabtype = $location->cabtypeComplete($sort, $Connection->con);
...
<html lang="en">
    <head>
        <title>Completed user page</title>
        <meta charset="utf-8">
